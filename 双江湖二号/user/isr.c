...
//    motor_control(2000,2000);
    gpt_get();
    if(RoadOut_flag||ZebraStop)//RoadOut_flag||RoadOutElec_flag||ZebraStop
    {
          Speed_Control(0 , 0);
          BeepON
    }
    else if(ZebraFlag)
    {
        if(ZebraFlag==21)
        {
            pwm_set_duty(S_MOTOR_PIN, 650);
            motor_control(3000, 3000);
            BeepON
            if(gptR_Cir>0&&gptL_Cir>70)
            {
                  Speed_Control(0,0);
                  BeepOFF
            }\
        }
        else if(ZebraFlag==22)
        {
             pwm_set_duty(S_MOTOR_PIN, 550);
             Speed_Control(200, 70);
             BeepON
   ...
   {
    //        Speed_Control(speed_base,speed_base);
    //        BeepOFF
            if(gDir_MidFilter>0)
            {
                Speed_Control(speed_base,speed_base-speed_out);
            }
            else
            {
                Speed_Control(speed_base+speed_out,speed_base);
            }


    }

    //    motor_control(3000, 0);
}
